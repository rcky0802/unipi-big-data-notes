...
\begin{table}[H]
  ...
  \begin{tabular}{r r l r r r}
    \toprule
    & & & \multicolumn{2}{c}{\textbf{Time per elem.}} & \\
    \cmidrule(lr){4-5}
    \textbf{$N$} & \textbf{Size} & \textbf{Cache level} & \textbf{\texttt{sum()}} & \textbf{\texttt{jumping()}} & \textbf{Slowdown} \\
    \midrule
...
When running this computation on two distinct vectors \texttt{V} and \texttt{West}, the sequential approach (calling \texttt{do\_more\_stuff\_4} on \texttt{V} and then on \texttt{West}) requires total time $2T$. If instead a single function processes both vectors within the same iteration, alternating the two independent chains:
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l r Y}
    \toprule
    \textbf{Configuration} & \textbf{Time / iter.} & \textbf{Branch predictor behavior} \\
    \midrule
