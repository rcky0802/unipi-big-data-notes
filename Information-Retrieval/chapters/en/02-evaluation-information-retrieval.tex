...
Rigorous performance evaluation is essential for designing, fine-tuning, and deploying Information Retrieval systems~\cite{manning2008,buttcher2010}. Search engine quality is evaluated across two complementary dimensions:
\begin{enumerate}
...
\begin{tikzpicture}[
    ...
    \node[eval] (assess) at (11.8, 1.45) {\textbf{Human Assessors}\\Relevance Judgments\\(Relevant / Non-rel)};
    ...
    \draw[-Stealth, very thick, orange!80!black] (poolbox.east) -- node[above=4pt, font=\scriptsize\bfseries, text=orange!90!black] {Pooled documents only} (assess.west);
\end{tikzpicture}
...
\begin{tikzpicture}
    ...
    \node[anchor=south west, font=\footnotesize, text=gray!75!black] at (-5.5, -3.05) {\textbf{TN} (True Negative): Non-relevant and not retrieved documents};
\end{tikzpicture}
...
\textbf{Limitations of $P@k$:} It does not reward placing relevant documents higher within the top-$k$ window, and it does not average cleanly across queries with fewer than $k$ total relevant documents. Similarly, $Recall@k$ ($R@k$) evaluates the fraction of all relevant documents recovered within the top $k$.

\subsection{Average Precision (AP)}
\textbf{Average Precision} computes precision at each rank position where a relevant document appears, rewarding systems that place relevant documents earlier in the ranking:
...
\begin{example}[Step-by-Step Calculation from Slides]
Consider a query with $R = 3$ total relevant documents. The search engine returns relevant documents at ranks $k_1 = 1$, $k_2 = 3$, and $k_3 = 5$.
The corresponding precisions are:
\[
...
\begin{property}[Macro-Averaging in MAP]
MAP implements a \textbf{macro-averaging} scheme: each query contributes equally ($1/|\mathcal{Q}|$) regardless of whether it has 2 or 200 relevant documents. It remains the gold standard in academic IR benchmarking for binary relevance.
\end{property}
...
In practical Web and commercial applications, binary judgments prove inadequate. Documents exhibit varying degrees of utility, typically evaluated on discrete integer scales (e.g., $0 = \text{Non-Relevant}$, $1 = \text{Marginal}$, $2 = \text{Relevant}$, $3 = \text{Highly Relevant}$).
...
\end{equation}
This formulation places heavy emphasis on surfacing top-tier relevance ($r=3 \implies 2^3 - 1 = 7$, compared to $r=1 \implies 2^1 - 1 = 1$).
...
\subsection{Complete Numerical Walkthrough from Slides}
Consider the 10-document ranking from the course lecture slides, judged on a scale of $0$--$3$:
\[
