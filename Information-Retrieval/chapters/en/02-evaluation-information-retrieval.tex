...
\begin{tikzpicture}[
    ...
    %--- Column labels ---
    \node[lbl, above=0.25cm of dc] {\textbf{Offline Phase}};
    \node[lbl, above=0.25cm of q]  {\textbf{Online Phase}};
...
The rise of generative conversational search engines and Large Language Models (LLMs) fundamentally alters traditional metrics such as CTR and Dwell Time: users consume direct synthesized summaries rather than navigating outgoing links. Consequently, factual grounding, hallucination minimization, and source attribution fidelity supplant traditional link click telemetry.

Several critical methodological debates have emerged regarding the evaluation of these systems:
\begin{itemize}
    \item \textbf{Number of conversational turns as a failure proxy:} In a generative dialogue system, an excessive number of turns required to satisfy an information need often signals that the system failed to comprehend the user's initial intent, forcing repeated clarifications or reformulation loops. Traditional Dwell Time thus yields to conversational length and session efficiency metrics.
    \item \textbf{Prompting variance and reproducibility:} Unlike keyword queries in classical IR, slight changes in prompt wording to a generative LLM can induce substantial variance in response quality. This intrinsic sensitivity severely complicates standardized, reproducible benchmarking.
    \item \textbf{Ontological difference between retrieval and synthesis:} Traditional IR systems \emph{retrieve} and rank preexisting documents from a known collection; generative LLMs \emph{synthesize} original probabilistic text. In retrieval, relevance is evaluated against a fixed corpus and gold-standard ground truth; in synthesis, verifying factual validity and correctness requires entirely new semantic evaluation frameworks.
    \item \textbf{Retrieval-Augmented Generation (RAG):} The RAG paradigm addresses hallucination risks by conditioning generative synthesis on documents fetched via classical IR. Consequently, evaluation requires decoupled yet unified assessment: measuring both retrieval quality and synthesis fidelity.
\end{itemize}
\end{alertblock}
...
\begin{tikzpicture}[
    ...
    \node[box] (sys1) at (-5.55, 1.5) {System $S_1$\\\footnotesize (Run 1)};
    \node[box] (sys2) at (-5.55, 0.0) {System $S_2$\\\footnotesize (Run 2)};
    \node[box] (sysk) at (-5.55, -1.5) {System $S_m$\\\footnotesize (Run $m$)};

    \node[pool] (poolbox) at (-0.75, 0.0) {
        \textbf{Candidate}\\[1pt]
        \textbf{Pool $\mathcal{P}$}\\[4pt]
        $\mathcal{P} = \bigcup_{i=1}^m \text{Top-}k(S_i)$\\[4pt]
        \footnotesize\textcolor{black!65}{(deduplicated)}
    };
    ...
    \draw[-Stealth, very thick, orange!85!black] (poolbox.east) -- node[above=2pt, font=\tiny\bfseries, text=orange!90!black, align=center] {Pooled\\documents only} (assess.west);
\end{tikzpicture}
...
\subsection{Truncated Metrics: AP@K and MAP@K}
