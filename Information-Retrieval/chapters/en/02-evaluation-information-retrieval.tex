...
\caption{Comparative overview of classical historical evaluation test corpora (Slide 11).}
...
\begin{alertblock}{The Distortion of $P@k$ When Averaged Across Queries (Slide 28)}
...
\begin{tikzpicture}[
    ...
    \draw[-Stealth, thick, blue!80!black] (0.0, 0.7) -- node[above, font=\scriptsize] {Search Engine Output Order} (9.0, 0.7);
\end{tikzpicture}
\caption{Schematic representation of Average Precision calculation over 5 documents with $R=3$ total relevant items (Slide 22).}
\label{fig:ap-blocks-en}
\end{figure}

\begin{example}[Basic Calculation with $R=3$ Relevant Documents (Slide 22)]
...
\begin{example}[Comparative Analysis Between Two Rankings (Slide 23)]
...
\begin{itemize}
    ...
    \]
\end{itemize}
Table~\ref{tab:ap-comparison-slide23-en} presents the step-by-step evolution of Precision and Recall across all 10 ranks.
...
\caption{Detailed comparative evaluation of Average Precision between Ranking \#1 and Ranking \#2 (Slide 23).}
\label{tab:ap-comparison-slide23-en}
\end{table}
...
\begin{example}[Computing MAP Over Two Queries (Slide 24)]
...
Engines thus adopt rank-truncated formulations (\textbf{MAP@K}, Slide 28):
...
In practical Web and commercial applications, binary judgments prove inadequate. Consider the real-world Google search query shown in the lecture slides (Slide 31--32):
\begin{center}
...
\begin{alertblock}{Behavioral Meaning of the Logarithmic Discount (Slide 34)}
