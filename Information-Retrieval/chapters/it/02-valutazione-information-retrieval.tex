...
\caption{Panoramica comparativa dei principali corpora benchmark storici di valutazione (Slide 11).}
...
\begin{alertblock}{La Distorsione di $P@k$ nella Media tra Query (Slide 28)}
...
\end{tikzpicture}
\caption{Rappresentazione schematica del calcolo di Average Precision su 5 documenti con $R=3$ rilevanti totali (Slide 22).}
\label{fig:ap-blocks}
\end{figure}

\begin{example}[Calcolo di Base con $R=3$ Rilevanti (Slide 22)]
...
\begin{example}[Confronto Comparativo tra Due Ordinamenti (Slide 23)]
...
\begin{itemize}
    ...
    \]
\end{itemize}
La Tabella~\ref{tab:ap-comparison-slide23} traccia l'evoluzione rango per rango di Precisione e Richiamo per entrambi i sistemi.
...
\caption{Calcolo comparativo dettagliato di Average Precision tra Ranking \#1 e Ranking \#2 (Slide 23).}
\label{tab:ap-comparison-slide23}
\end{table}
...
\begin{property}[Macro-Averaging vs. Micro-Averaging (Slide 25)]
...
\begin{example}[Calcolo di MAP su Due Query (Slide 24)]
...
Si adotta pertanto la variante troncata a soglia fissa $K$ (\textbf{MAP@K}, Slide 28):
...
Nelle applicazioni Web reali, l'ipotesi che un documento sia puramente rilevante ($1$) o non rilevante ($0$) risulta insufficiente. Si consideri la query reale illustrata nelle slide del corso (Slide 31--32):
\begin{center}
...
\begin{alertblock}{Significato Comportamentale dello Sconto Logaritmico (Slide 34)}
