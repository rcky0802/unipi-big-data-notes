...
\begin{tikzpicture}[
    ...
    \node[label, above=0.25cm of dc]  {\textbf{Fase Offline}};
    \node[label, above=0.25cm of q]   {\textbf{Fase Online}};
...
\begin{itemize}
    ...
    \item \textbf{Indexing:} La pipeline di pre-elaborazione che trasforma i documenti grezzi in una rappresentazione indicizzabile: parsing HTML, tokenizzazione, normalizzazione, rimozione delle stopword e stemming\slash lemmatizzazione.
...
L'emergere di motori di ricerca generativi e assistenti basati su Large Language Models (LLM) ridefinisce radicalmente metriche consolidate come Dwell Time e CTR: l'utente non naviga pi\`u necessariamente attraverso una lista di link esterni, ma consuma sintesi testuali generate direttamente dal modello. In tale scenario, l'accuratezza fattuale, l'assenza di allucinazioni e la corretta attribuzione delle fonti rimpiazzano il conteggio dei clic tradizionali.

Il dibattito metodologico su come valutare questi sistemi solleva interrogativi cruciali:
\begin{itemize}
    \item \textbf{Numero di turni conversazionali come proxy di insoddisfazione:} In un sistema conversazionale, un elevato numero di turni necessari per soddisfare un bisogno informativo pu\`o segnalare che il sistema non ha compreso l'intento iniziale dell'utente, costringendolo a continue riformulazioni. Il Dwell Time perde quindi centralit\`a, sostituito dall'analisi dell'efficienza e della lunghezza della sessione di dialogo.
    \item \textbf{Varianza e instabilit\`a del prompting:} A differenza delle query lessicali dell'IR classico, lievi variazioni nella formulazione del prompt possono indurre forti discrepanze qualitative nella risposta dell'LLM. Questa elevata sensibilit\`a rende complessa la standardizzazione e la riproducibilit\`a sperimentale dei benchmark.
    \item \textbf{Differenza ontologica tra recupero e sintesi:} Un motore tradizionale si limita a \textit{recuperare} e ordinare documenti preesistenti da un corpus noto; un modello generativo \textit{sintetizza} invece un testo originale probabilistico. Mentre nel primo caso la rilevanza \`e misurabile rispetto a un insieme oggettivo di documenti rilevanti (\textit{ground truth}), nel secondo la correttezza fattuale della sintesi richiede verifiche semantiche inedite.
    \item \textbf{Il paradigma RAG (Retrieval-Augmented Generation):} L'architettura RAG vincola la generazione alla consultazione preventiva di documenti estratti da un indice IR tradizionale. In questo contesto, la valutazione diventa composita: occorre misurare separatamente e congiuntamente l'efficacia del recupero documentale e la fedelt\`a informativa del testo generato.
\end{itemize}
\end{alertblock}
...
\centering
\begin{tikzpicture}[
    box/.style={rectangle, rounded corners=4pt, draw=blue!75!black, fill=blue!6, thick, minimum width=2.3cm, minimum height=0.85cm, text width=2.2cm, align=center, font=\small},
    pool/.style={rectangle, rounded corners=5pt, draw=orange!85!black, fill=orange!8, thick, minimum width=3.5cm, minimum height=2.2cm, text width=3.3cm, align=center, font=\small, inner sep=5pt},
    eval/.style={rectangle, rounded corners=5pt, draw=teal!80!black, fill=teal!6, thick, minimum width=3.1cm, minimum height=2.2cm, text width=3.0cm, align=center, font=\small, inner sep=5pt}
]
    \node[box] (sys1) at (-5.55, 1.5) {Sistema $S_1$\\\footnotesize (Run 1)};
    \node[box] (sys2) at (-5.55, 0.0) {Sistema $S_2$\\\footnotesize (Run 2)};
    \node[box] (sysk) at (-5.55, -1.5) {Sistema $S_m$\\\footnotesize (Run $m$)};

    \node[pool] (poolbox) at (-0.75, 0.0) {
        \textbf{Pool dei}\\[1pt]
        \textbf{Candidati $\mathcal{P}$}\\[4pt]
        $\mathcal{P} = \bigcup_{i=1}^m \text{Top-}k(S_i)$\\[4pt]
        \footnotesize\textcolor{black!65}{(senza duplicati)}
    };

    \node[eval] (assess) at (5.15, 0.0) {
        \textbf{Giudici Umani}\\[4pt]
        Valutazione manuale\\
        della rilevanza\\[4pt]
        \footnotesize\textit{(Relevant / Non-rel)}
    };

    \draw[-Stealth, thick, blue!75!black] (sys1.east) -- node[above, sloped, font=\scriptsize] {Top-$k$} (poolbox.155);
    \draw[-Stealth, thick, blue!75!black] (sys2.east) -- node[above, font=\scriptsize] {Top-$k$} (poolbox.west);
    \draw[-Stealth, thick, blue!75!black] (sysk.east) -- node[below, sloped, font=\scriptsize] {Top-$k$} (poolbox.205);
    \draw[-Stealth, very thick, orange!85!black] (poolbox.east) -- node[above=2pt, font=\tiny\bfseries, text=orange!90!black, align=center] {Solo documenti\\del pool $\mathcal{P}$} (assess.west);
\end{tikzpicture}
