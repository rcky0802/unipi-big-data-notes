...
\begin{itemize}
    \item \textbf{Interpretability:} Produces readable categories (e.g., $\{ \text{Young}, \text{Adult}, \text{Senior} \}$).
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Example:} A feature with $m=4$ states ($\{\text{Red}, \text{Blue}, \text{Green}, \text{Yellow}\}$) requires $\lceil \log_2(4) \rceil = 2$ bits:
        \begin{equation}
    ...
    For an attribute with $m$ distinct categories, $m$ separate binary indicator variables $x_1, \dots, x_m$ are introduced, where $x_j = 1$ if the instance assumes modality $j$, and $x_j = 0$ otherwise (Table~\ref{tab:binarization-example}).
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Currency conversion: $\text{Lira} = 1936.27 \times \text{Euro} \quad (p=1, a=1936.27, b=0)$.
    ...
    \begin{itemize}
        ...
        \item \textit{Transformed Series:} Applying $y = \log_{10}(\text{Gain})$ maps the entire dynamic range into $[0.6990, \; 4.0414]$. Multiplicative scales (orders of magnitude) become uniform linear intervals, stabilizing variance and restoring distributional balance (Figure~\ref{fig:log-transform-compression}).
    \end{itemize}
    \end{example}
