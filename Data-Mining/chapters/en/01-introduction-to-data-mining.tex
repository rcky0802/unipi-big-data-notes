...
In this modern scenario, applied Artificial Intelligence rests on the foundational paradigm:
\begin{equation}
\mathbf{AI} = \mathbf{Machine\ Learning} + \mathbf{Big\ Data}
\end{equation}
where the availability of massive databases (\textit{Big Data}) provides the empirical substrate and source of information, while \textit{Machine Learning} and \textit{Data Mining} algorithms act as the logical-inductive engine capable of extracting non-trivial generalizations and decision patterns.
...
\section{The KDD Process (Knowledge Discovery in Databases)}
...
\begin{example}[Didactic Example: Decision Tree Induction for Credit Worthiness]
Consider a standard bank credit lending problem, where the objective is to predict whether a new applicant is financially reliable (\textit{Credit Worthy} $\in \{\text{Yes}, \text{No}\}$).

\paragraph{1. Training Set (Historical data with known class labels):}
The lending institution has historical records of past customers with observed loan repayment outcomes:
\begin{table}[htbp]
\centering
\begin{tabular}{cllcc}
\toprule
\textbf{TID} & \textbf{Employed} & \textbf{Level of Education} & \textbf{\# Years at Address} & \textbf{Credit Worthy (Class)} \\
\midrule
1 & Yes & Graduate & 5 & \textbf{Yes} \\
2 & Yes & High School & 2 & \textbf{No} \\
3 & No & Undergrad & 1 & \textbf{No} \\
4 & Yes & High School & 10 & \textbf{Yes} \\
\bottomrule
\end{tabular}
\caption{Training set used to train the credit classification model.}
\label{tab:credit-train-en}
\end{table}

\paragraph{2. Learned Model (Decision Tree):}
The classification algorithm extracts a hierarchical sequence of logical decisions from the historical data (Fig.~\ref{fig:decision-tree-example-en}):
...
\begin{tikzpicture}[
    ...
    % Root
    \node[decision] (root) at (0, 0) {Employed?};

    % Level 1
    \node[leaf_no] (l_no) at (-3.4, -1.5) {Class = No};
    \node[decision] (l_edu) at (2.6, -1.5) {Education Level?};

    % Level 2
    \node[leaf_yes] (l_grad) at (0.6, -3.1) {Class = Yes};
    \node[decision] (l_years) at (4.8, -3.1) {Years at Address?};

    % Level 3
    \node[leaf_yes] (l_yyes) at (3.2, -4.7) {Class = Yes};
    \node[leaf_no] (l_yno) at (6.4, -4.7) {Class = No};

    % Edges
    \draw[-Stealth, thick] (root) -- node[edge_label] {No} (l_no);
    \draw[-Stealth, thick] (root) -- node[edge_label] {Yes} (l_edu);

    \draw[-Stealth, thick] (l_edu) -- node[edge_label] {Graduate} (l_grad);
    \draw[-Stealth, thick] (l_edu) -- node[edge_label] {\{High School, Undergrad\}} (l_years);

    \draw[-Stealth, thick] (l_years) -- node[edge_label] {$> 3$ yrs} (l_yyes);
    \draw[-Stealth, thick] (l_years) -- node[edge_label] {$\le 3$ yrs} (l_yno);
\end{tikzpicture}
\caption{Classifier model: learned decision tree.}
\label{fig:decision-tree-example-en}
\end{figure}

\paragraph{3. Test Set (Unseen new instances):}
When new credit applications arrive with unknown class labels (\texttt{?}), the model evaluates them by traversing the decision tree:
\begin{table}[htbp]
\centering
\small
\begin{tabular}{cllccl}
\toprule
\textbf{TID} & \textbf{Employed} & \textbf{Education} & \textbf{Years} & \textbf{Target} & \textbf{Model Prediction} \\
\midrule
5 & Yes & Undergrad & 7 & \textbf{?} & \textbf{Yes} (Employed $\to$ Undergrad $\to 7 > 3$) \\
6 & No & Graduate & 3 & \textbf{?} & \textbf{No} (Unemployed) \\
7 & Yes & High School & 2 & \textbf{?} & \textbf{No} (Employed $\to$ High School $\to 2 \le 3$) \\
\bottomrule
\end{tabular}
\caption{Evaluation of the test set using the learned decision tree.}
\label{tab:credit-test-en}
\end{table}
This example highlights that the classifier does not merely memorize the training instances, but generalizes predictive logical rules applicable to any novel instance.
\end{example}
...
\begin{tikzpicture}[scale=0.95]
    ...
    \draw[Stealth-Stealth, thick, blue!80!black] (-4.1, -0.5) -- node[above=2pt, font=\scriptsize\bfseries, text=blue!80!black] {min intra-distance} (-2.3, -0.6);
    ...
    \draw[Stealth-Stealth, thick, green!50!black] (2.3, -0.6) -- node[below=2pt, font=\scriptsize\bfseries, text=green!60!black] {min intra-distance} (4.3, -0.5);

    % Inter-cluster distance arrow
    \draw[Stealth-Stealth, very thick, red!80!black] (-1.6, 0.2) -- node[above=3pt, font=\small\bfseries, text=red!80!black] {max inter-distance} (1.6, 0.2);
\end{tikzpicture}
...
\begin{itemize}
    \item \textbf{Market Segmentation:} Partitioning customers by demographic traits and purchasing habits to target differentiated marketing campaigns.
