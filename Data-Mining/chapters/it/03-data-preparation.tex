...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textit{Esempio:} Un attributo con $m=4$ stati ($\{\text{Rosso}, \text{Blu}, \text{Verde}, \text{Giallo}\}$) richiede $\lceil \log_2(4) \rceil = 2$ bit:
        \begin{equation}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{Serie trasformata:} Applicando la funzione $y = \log_{10}(\text{Gain})$, l'intera escursione numerica viene ricondotta nell'intervallo $[0.6990, \; 4.0414]$. Le differenze moltiplicative (ordini di grandezza) diventano distanze lineari omogenee, stabilizzando la varianza e ripristinando la simmetria distributiva (Figura~\ref{fig:log-transform-compression}).
    \end{itemize}
    \end{example}
